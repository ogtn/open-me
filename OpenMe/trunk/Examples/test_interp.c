...
static void test_main(void)
{
    ...
    /* retreive the vars from the environment */
    {
        ...
        assert(omeConsoleGetString(c, "test_string", &str_3) == OME_CONSOLE_STATUS_NO_ERROR);
...
static void cb_create(omeConsole *c, void *user_data)
{
    ...
    const char *type = c->cmdLine.args[0];
    const char *name = c->cmdLine.args[1];

    if(c->cmdLine.argsNb < 2)
    {
    ...
    if(strcmp(type, "int") == 0)
        status = omeConsoleRegisterInt(c, name, 0); 
    else if(strcmp(type, "float") == 0)
        status = omeConsoleRegisterInt(c, name, 0.f);
    else if(strcmp(type, "string") == 0)
        status = omeConsoleRegisterString(c, name, "nothing yet!");
    else
    {
...
int main(void)
{
    ...
    omeConsoleRegisterCallback(console, "ls", cb_list, NULL);
